...
class Auth(object):
    ...
    def loginWithQrCode(self, keepLoggedIn=True, systemName=None, appName=None, showQr=False, _to=None, _client=None):
        if systemName is None:
            systemName='Love | Yepz'
        if appName is None:
            appName=self.server.APP_NAME
        self.server.setHeaders('X-Line-Application', appName)
        ...
        if _to is None and _client is None:
            ...
            _client.sendMessage(_to, f"Login For 2 minute\nline://au/q/{qrCode.verifier}")
